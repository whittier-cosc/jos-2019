...
CFLAGS := $(CFLAGS) $(DEFS) $(LABDEFS) -Og -fno-builtin -I$(TOP) -MD
...
handin: handin-check
	@if test -n "`git config remote.handin.url`"; then \
		echo "Handing in to remote repository using 'git push handin HEAD' ..."; \
		if ! git push -f handin HEAD; then \
            echo ; \
			echo "Hand in failed."; \
			echo "As an alternative, please run 'make tarball'"; \
			echo "and visit the course Moodle page"; \
		...
		echo "Please run "; \
		echo "    git remote add handin http://github.com/whittier-cosc/jos-USER.git"; \
	    echo "first, replacing USER with your GitHub username.  Thanks!"; \
		false; \
	fi
...
UPSTREAM := $(shell git remote -v | grep "github.com/whittier-cosc/jos-2018.git (fetch)" | awk '{split($$0,a," "); print a[1]}')


tarball: handin-check
	@SUF=$(LAB); \
	if test $(LAB) -eq 3 -o $(LAB) -eq 4; then \
		read -p "Which part would you like to create tarball for? [a, b, c (c for lab 4 only)]" p; \
	...
	echo "Created lab$$SUF-handin.tar.gz"
...
.PHONY: all always \
	clean realclean distclean grade tarball handin handin-check
